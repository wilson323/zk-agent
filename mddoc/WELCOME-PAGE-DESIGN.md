  ...
  return (
    ...
    >
      <div className='relative bg-[var(--card-bg)] backdrop-blur-xl border border-[var(--card-border)] rounded-3xl p-8 shadow-[var(--card-shadow)] hover:shadow-[var(--card-shadow-hover)] transition-all duration-500 overflow-hidden'>
        ...
        {/* 背景渐变装饰 */}
        <div
          className={`absolute inset-0 bg-gradient-to-br ${agent.gradient} opacity-5 group-hover:opacity-15 transition-opacity duration-500`}
        />
        ...
        <div className='relative z-10 text-center'>
          ...
          <p className='text-text-secondary mb-6 leading-relaxed text-sm'>{agent.description}</p>
          ...
          >
            <span className='relative z-10 flex items-center justify-center'>
              立即体验
              <motion.span animate={{ x: isHovered ? 5 : 0 }} className='ml-2'>
                →
              </motion.span>
            </span>
...
const useFrameScheduling = (particles: Particle[], frameLimit: number = 16) => {
  ...
  useEffect(() => {
    ...
    const renderFrame = () => {
      ...
      setVisibleParticles(prev => [...prev, ...particles.slice(startIndex, endIndex)]);
...
const useImageOptimization = () => {
  // 预加载关键资源
  useEffect(() => {
    const preloadImages = ['/images/hero-bg.webp', '/images/agent-icons.webp'];
