    ...
    <div className='face-enhancement-uploader'>
      ...
      <div {...getRootProps()} className={`dropzone ${isDragActive ? 'active' : ''}`}>
        ...
        {uploadStatus === 'idle' && (
          <div className='upload-prompt'>
            ...
            <p>拖拽照片到这里，或点击选择文件</p>
            <p className='text-sm text-gray-500'>支持 JPG, PNG, WEBP 格式，最大 20MB</p>
          </div>
        )}
      </div>
      ...
      {(uploadStatus === 'uploading' || uploadStatus === 'processing') && (
        <div className='processing-status'>
          <CircularProgressbar value={progress} text={`${progress}%`} className='w-24 h-24' />
          <p>{uploadStatus === 'uploading' ? '上传中...' : '增强处理中...'}</p>
        </div>
      )}

      {/* 结果展示 */}
      {uploadStatus === 'completed' && result && (
        <ResultDisplay result={result} onSave={handleSaveImage} onClear={handleClearResult} />
      )}
...
const ResultDisplay = ({ result, onSave, onClear }) => {
  ...
  return (
    <div className='result-display'>
      ...
      {showComparison ? (
        <ImageComparison original={result.original_image} enhanced={result.enhanced_image} />
      ) : (
        <img src={result.enhanced_image_url} alt='增强后的照片' className='enhanced-image' />
      )}
...
const BatchUploader = () => {
  ...
  return (
    <div className='batch-uploader'>
      ...
      {/* 批量处理进度 */}
      {batchStatus === 'processing' && <BatchProgress progress={batchProgress} />}
    </div>
  );
};
```
