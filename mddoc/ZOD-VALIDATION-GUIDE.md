...
```typescript
import { withValidation, withSecurity, combineMiddleware } from '@/lib/validation/middleware';
...
// 自动清理和验证输入
const result = sanitizeAndValidate(UserInputSchema, userInput, [
...
```typescript
const customCADValidator = z.custom<File>(
  file => {
    // 自定义CAD文件验证逻辑
