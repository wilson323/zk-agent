...
export class ApiResponseWrapper {
   ...
   * 创建错误响应
   */
  static error(code: ErrorCode, message: string, details: any = null, status = 400): NextResponse {
 ...
 * 创建API路由处理器
 */
export function createApiRoute<T = any>(handler: RouteHandler<T>, options: RouteOptions<T> = {}) {
  return async (
    request: NextRequest,
    ...
    try {
      ...
      if (options.requireAuth) {
        ...
        if (!token) {
          return ApiResponseWrapper.error(ErrorCode.UNAUTHORIZED, '需要认证令牌', null, 401);
        }

        const payload = verifyToken(token);
        if (!payload) {
          return ApiResponseWrapper.error(ErrorCode.UNAUTHORIZED, '无效的认证令牌', null, 401);
        }
        ...
        if (options.requireAdmin && context.user.role !== 'admin') {
          return ApiResponseWrapper.error(ErrorCode.FORBIDDEN, '需要管理员权限', null, 403);
        }
      }
...
export function handleApiError(error: unknown): NextResponse {
  if (error instanceof z.ZodError) {
    return ApiResponseWrapper.error(ErrorCode.VALIDATION_ERROR, '数据验证失败', error.errors, 400);
  }
  ...
  return ApiResponseWrapper.error(ErrorCode.INTERNAL_ERROR, '未知错误', undefined, 500);
}
