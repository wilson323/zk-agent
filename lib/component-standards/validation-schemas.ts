 ...
 */
export const sizeVariantSchema = z.enum(['xs', 'sm', 'default', 'lg', 'xl', '2xl']);

/**
 * 圆角变体验证
 * 遵循 Tailwind CSS 圆角系统
 */
export const radiusVariantSchema = z.enum(['none', 'sm', 'default', 'md', 'lg', 'xl', 'full']);
 ...
 * 遵循 WAI-ARIA 1.2 规范
 */
export const ariaPropsSchema = z
  .object({
...
export const validationRulesSchema = z.object({
  ...
  pattern: z
    .union([z.instanceof(RegExp), z.object({ value: z.instanceof(RegExp), message: z.string() })])
    .optional(),
 ...
 */
export const breadcrumbPropsSchema = z
  .object({
    items: z.array(
      z.object({
        title: z.string(),
...
export const notificationPropsSchema = z.object({
  ...
  placement: z
    .enum(['topLeft', 'topRight', 'bottomLeft', 'bottomRight', 'top', 'bottom'])
    .default('topRight'),
  icon: z.any().optional(),
 ...
 * @returns 属性检查函数
 */
export function createRuntimePropsChecker<T>(schema: z.ZodSchema<T>, componentName: string) {
  return (props: unknown): T => {
...
export function validateResponsiveProps(props: Record<string, any>) {
  ...
  Object.entries(props).forEach(([key, value]) => {
    if (typeof value === 'object' && value !== null) {
      const invalidBreakpoints = Object.keys(value).filter(bp => !breakpoints.includes(bp));

      if (invalidBreakpoints.length > 0) {
        errors.push(`${key} 包含无效的断点: ${invalidBreakpoints.join(', ')}`);
      }
    }
  });
