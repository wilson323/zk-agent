...
export function useStableCallback<T extends (...args: any[]) => any>(
  ...
  return React.useCallback(((...args: any[]) => callbackRef.current(...args)) as T, stableDeps);
}
 ...
 * React Hook: 内容懒加载
 */
export function useContentLazyLoad<T extends HTMLElement>(options: LazyLoadOptions = {}) {
  const { threshold = 0.1, rootMargin = '50px' } = options;
  ...
  React.useEffect(() => {
    ...
    return () => {
      ...
      if (process.env.NODE_ENV === 'development') {
        if (renderTime > 16) {
          // 超过一帧时间
          logger.warn(`${componentName} 渲染时间过长: ${renderTime.toFixed(2)}ms`);
        }
