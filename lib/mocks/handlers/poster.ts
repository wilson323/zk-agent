...
export const posterHandlers = [
  ...
  http.get('/api/poster/templates/:id', ({ params }) => {
    ...
    if (!template) {
      return HttpResponse.json({ success: false, message: '模板不存在' }, { status: 404 });
    }
  ...
  http.get('/api/poster/tasks/:id', ({ params }) => {
    ...
    if (!task) {
      return HttpResponse.json({ success: false, message: '生成任务不存在' }, { status: 404 });
    }
  ...
  http.get('/api/poster/categories', () => {
    const categories = [
      ...
      },
      {
        id: 'event',
        name: '活动',
        count: mockTemplates.filter(t => t.category === 'event').length,
      },
      {
        id: 'social',
        name: '社交',
