...
export const cadHandlers = [
  ...
  http.post('/api/cad/upload', async ({ request }) => {
    ...
    if (!file) {
      return HttpResponse.json({ success: false, message: '请选择文件' }, { status: 400 });
    }
    ...
    if (!fileExtension || !allowedTypes.includes(fileExtension)) {
      return HttpResponse.json({ success: false, message: '不支持的文件格式' }, { status: 400 });
    }
  ...
  http.get('/api/cad/analyses/:id', ({ params }) => {
    ...
    if (!result) {
      return HttpResponse.json({ success: false, message: '分析结果不存在' }, { status: 404 });
    }
  ...
  http.get('/api/cad/analyses/:id/progress', ({ params }) => {
    ...
    if (!result) {
      return HttpResponse.json({ success: false, message: '分析任务不存在' }, { status: 404 });
    }
  ...
  http.get('/api/cad/analyses/:id/export', ({ params, request }) => {
    ...
    if (result.status !== 'completed') {
      return HttpResponse.json({ success: false, message: '分析尚未完成' }, { status: 400 });
    }
