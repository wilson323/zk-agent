...
export const authHandlers = [
  ...
  http.post('/api/auth/login', async ({ request }) => {
    ...
    if (!user) {
      return HttpResponse.json({ success: false, message: '用户不存在' }, { status: 401 });
    }

    if (!user.isActive) {
      return HttpResponse.json({ success: false, message: '账户已被禁用' }, { status: 401 });
    }

    // 模拟密码验证（实际应用中应该使用加密密码）
    if (body.password !== 'demo123') {
      return HttpResponse.json({ success: false, message: '密码错误' }, { status: 401 });
    }
  ...
  http.post('/api/auth/register', async ({ request }) => {
    ...
    if (existingUser) {
      return HttpResponse.json({ success: false, message: '邮箱已被注册' }, { status: 400 });
    }

    // 检查用户名是否已存在
    const existingUsername = mockUsers.find(u => u.username === body.username);
    if (existingUsername) {
      return HttpResponse.json({ success: false, message: '用户名已被使用' }, { status: 400 });
    }
  ...
  http.get('/api/auth/me', ({ request }) => {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return HttpResponse.json({ success: false, message: '未授权访问' }, { status: 401 });
    }

    if (!currentUser) {
      return HttpResponse.json({ success: false, message: '用户未登录' }, { status: 401 });
    }
  ...
  http.post('/api/auth/change-password', async ({ request }) => {
    ...
    if (body.currentPassword !== 'demo123') {
      return HttpResponse.json({ success: false, message: '当前密码错误' }, { status: 400 });
    }
  ...
  http.get('/api/auth/usage', ({ request }) => {
    ...
    const usagePercentage = {
      ...
      posterGenerations:
        (currentUser.usage.posterGenerations / currentUser.limits.posterGenerations) * 100,
  ...
  http.post('/api/auth/forgot-password', async ({ request }) => {
    ...
    if (!user) {
      return HttpResponse.json({ success: false, message: '邮箱不存在' }, { status: 404 });
    }
  ...
  http.post('/api/auth/reset-password', async ({ request }) => {
    ...
    if (!body.token || body.token.length < 10) {
      return HttpResponse.json({ success: false, message: '无效的重置令牌' }, { status: 400 });
    }
