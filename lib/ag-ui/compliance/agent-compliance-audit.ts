...
} from '../protocol/agent-compliance-checker';
...
export class AgentComplianceAudit {
  ...
  async performAudit(
    agentId: string,
    options: { type?: string; detailed?: boolean } = {}
  ...
  private async auditConversationAgent(): Promise<ComplianceReport> {
    ...
    try {
      ...
      // 生成合规性报告
      const report = AgentComplianceChecker.generateComplianceReport(
        'conversation',
        agent,
        events,
        messages
   ...
   * 计算总体合规性
   */
  private calculateOverallCompliance(reports: ComplianceReport[]): {
    score: number;
    isCompliant: boolean;
  } {
    if (reports.length === 0) {
