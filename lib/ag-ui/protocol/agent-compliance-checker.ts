...
export class AgentComplianceChecker {
  ...
  static checkAgentDefinition(agent: AgentDefinition): ComplianceResult {
    ...
    if (!agent.instructions) {
      errors.push('智能体指令不能为空');
    }
    if (!agent.model) {
      errors.push('智能体模型不能为空');
    }
    ...
    if (!agent.metadata?.protocolVersion) {
      ...
      warnings.push(
        `协议版本不匹配，当前: ${agent.metadata.protocolVersion}, 期望: ${AG_UI_PROTOCOL_VERSION}`
      );
    }
  ...
  static checkMessageFormat(message: Message): ComplianceResult {
    ...
    if (!message.role) {
      errors.push('消息角色不能为空');
    }
    if (message.content === undefined || message.content === null) {
      errors.push('消息内容不能为空');
    }
    if (!message.timestamp) {
      errors.push('消息时间戳不能为空');
    }
  ...
  static checkEventFormat(event: AgUIEvent): ComplianceResult {
    ...
    switch (event.type) {
      ...
      case 'run-finished':
      case 'run-error':
        if (!(event as any).threadId) {
          errors.push('运行事件必须包含threadId');
        }
        if (!(event as any).runId) {
          errors.push('运行事件必须包含runId');
        }
        break;
      ...
      case 'text-message-end':
        if (!(event as any).messageId) {
          errors.push('消息事件必须包含messageId');
        }
        break;
      ...
      case 'tool-call-end':
        if (!(event as any).toolCallId) {
          errors.push('工具调用事件必须包含toolCallId');
        }
        break;
    }
  ...
  private static generateRecommendations(
    definitionCheck: ComplianceResult,
    runtimeCheck: ComplianceResult
  ): string[] {
