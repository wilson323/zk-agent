...
export class ToolRegistry {
    ...
    const toolDefinition: Tool = {
      ...
      function: {
        ...
        parameters: {
          ...
          required: Object.keys(executor.parameters).filter(
            key => executor.parameters[key].required === true
          ),
        },
      },
    };
  ...
  async execute(name: string, args: any, context?: any): Promise<any> {
    ...
    try {
      ...
      throw new Error(
        `Tool execution failed for ${name}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
