...
export class AgUIAgentManager implements IAgentManager {
  ...
  async createAgentFromFastGPT(params: CreateAgentParams): Promise<AgentDefinition> {
    ...
    // 输入验证
    if (!appId?.trim()) {
      throw new AgentValidationError('AppId is required and cannot be empty', [
        'appId is required',
      ]);
    }
    if (!apiKey?.trim()) {
      throw new AgentValidationError('API Key is required and cannot be empty', [
        'apiKey is required',
      ]);
    }
    ...
    try {
      ...
      if (!appData || typeof appData !== 'object') {
        throw new AgentValidationError('Invalid response structure from FastGPT API', [
          'Invalid FastGPT API response format',
        ]);
      }
  ...
  validateAgent(agent: AgentDefinition): AgentValidationResult {
    ...
    if (agent.tools && Array.isArray(agent.tools)) {
      agent.tools.forEach((tool: Tool, index: number) => {
    ...
    // 添加警告检查
    if (agent.config?.temperature && (agent.config.temperature < 0 || agent.config.temperature > 2)) {
      warnings.push('温度参数建议在0-2之间');
    }
    if (agent.config?.maxTokens && agent.config.maxTokens > 4096) {
