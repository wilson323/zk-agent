...
export class ErrorTracker extends EventEmitter {
  ...
  private determineSeverity(errors: any[]): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
    ...
    const hasHighLevelErrors = errors.some(
      e => e.level === LogLevel.FATAL || e.level === LogLevel.ERROR
    );
    const affectedUsers = new Set(errors.map(e => e.context.userId).filter(Boolean)).size;

    if (frequency > 100 || affectedUsers > 50 || hasHighLevelErrors) {
      return 'CRITICAL';
    }
    if (frequency > 50 || affectedUsers > 20) {
      return 'HIGH';
    }
    if (frequency > 10 || affectedUsers > 5) {
      return 'MEDIUM';
    }
    return 'LOW';
  }
  ...
  private extractCommonPatterns(errors: any[]): string[] {
    ...
    if (urls.length > 0) {
      ...
      if (commonUrl) {
        patterns.push(`Common URL: ${commonUrl}`);
      }
    }
    ...
    if (userAgents.length > 0) {
      ...
      if (commonUA) {
        patterns.push(`Common User Agent: ${commonUA}`);
      }
    }
   ...
   * 生成建议操作
   */
  private generateSuggestedActions(errorType: string, _errors: any[]): string[] {
  ...
  private startCacheCleanup(): void {
    this.cacheCleanupInterval = setInterval(() => {
      ...
      const cacheEntries = Array.from(this.errorCache.entries());
      for (let i = 0; i < cacheEntries.length; i++) {
        const [fingerprint, errors] = cacheEntries[i];
