...
export class RootCauseAnalyzer {
  ...
  private async findRelatedErrors(errorReport: ErrorReport): Promise<ErrorReport[]> {
    try {
      ...
      // 查找时间窗口内的相关错误
      const relatedLogs = await prismaClient.errorLog.findMany({
        where: {
          createdAt: {
            gte: startTime,
            lte: endTime,
  ...
  private async identifyPatterns(
    ...
    Object.entries(byType).forEach(([type, errors]) => {
      const typedErrors = errors as ErrorReport[];
      if (typedErrors.length > 1) {
        patterns.push({
          pattern: `Error Type: ${type}`,
          frequency: typedErrors.length,
          timeRange: {
            start: new Date(Math.min(...typedErrors.map((e: ErrorReport) => e.timestamp.getTime()))),
            end: new Date(Math.max(...typedErrors.map((e: ErrorReport) => e.timestamp.getTime()))),
          },
          commonContext: this.extractCommonContext(typedErrors),
          severity: typedErrors[0].error?.severity || ErrorSeverity.LOW,
        });
      }
    });
  ...
  private identifyContributingFactors(
    ...
    const timeSpan =
      relatedErrors.length > 0
        ...
        Math.min(...relatedErrors.map(e => e.timestamp.getTime()))
        : 0;
  ...
  private async assessImpact(
    ...
    } else if (affectedSystems.length > 3) {
  ...
  private estimateDowntime(errors: ErrorReport[]): number {
    ...
    const timeSpan =
      Math.max(...errors.map((e: ErrorReport) => e.timestamp.getTime())) -
  ...
  private limitCacheSize(): void {
    if (this.analysisCache.size > this.maxCacheSize) {
      const entries = Array.from(this.analysisCache.entries()).sort(
        (a, b) => a[1].analysisTimestamp.getTime() - b[1].analysisTimestamp.getTime()
      );
