...
export class ExportSystem {
   ...
   * 导出海报
   */
  async exportPoster(template: PosterTemplate, userContent: UserContent, config: ExportConfig): Promise<ExportResult> {
    ...
    try {
      ...
      return {
        ...
        error: z.instanceof(Error).safeParse(error).success ? (error as Error).message : 'Unknown error',
      };
    }
  }
  ...
  private async exportRasterImage(
    ...
    // ============================================================================
    const dataUrl = this.canvas.toDataURL(mimeType, config.quality);
   ...
   * 渲染海报到画布
   */
  private async renderPosterToCanvas(
    template: PosterTemplate,
    userContent: UserContent,
    config: ExportConfig,
  ): Promise<void> {
   ...
   * 渲染背景
   */
  private async renderBackground(template: PosterTemplate, config: ExportConfig): Promise<void> {
    if (!this.ctx) {
      return;
    }

    // 查找背景元素
    const bgElement = (template as any)?.elements?.find((el: any) => el.type === 'background');
    if (!bgElement) {
      // 默认白色背景
   ...
   * 渲染元素
   */
  private async renderElement(element: PosterElement, userContent: UserContent, config: ExportConfig): Promise<void> {
    if (!this.ctx) {
      return;
    }

    const elementData = element as any;
    const { position, style, content, type } = elementData;
   ...
   * 生成SVG内容
   */
  private generateSVG(template: PosterTemplate, userContent: UserContent, config: ExportConfig): string {
   ...
   * 元素转SVG
   */
  private elementToSVG(element: PosterElement, userContent: UserContent, config: ExportConfig): string {
  ...
  private async compressImage(blob: Blob, config: ExportConfig): Promise<Blob> {
    try {
      ...
      logger.error('Image compression failed:', { data: error });
  ...
  private static readonly DEFAULT_WATERMARK_SIZE = 50;
  ...
  private getDefaultConfig(): ExportConfig {
    return {
      format: 'png',
      quality: 0.9,
      width: ExportSystem.DEFAULT_WIDTH,
      height: ExportSystem.DEFAULT_HEIGHT,
      dpi: ExportSystem.DEFAULT_DPI,
      compression: true,
      watermark: {
        enabled: false,
        position: 'bottom-right',
        opacity: 0.5,
        size: ExportSystem.DEFAULT_WATERMARK_SIZE,
      },
    };
  }
