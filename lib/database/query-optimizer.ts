...
export class QueryOptimizer extends EventEmitter {
   ...
   * @param parameters 查询参数
   */
  private recordSlowQuery(sql: string, executionTime: number, parameters?: any[]): void {
  ...
  public getPerformanceReport() {
    ...
    const totalErrors = Array.from(this.queryStats.values()).reduce(
      (sum, stats) => sum + stats.errorCount,
      0
    );

    const avgExecutionTime =
      Array.from(this.queryStats.values()).reduce((sum, stats) => sum + stats.avgExecutionTime, 0) /
      this.queryStats.size;
    ...
    const queryTypeDistribution = Array.from(this.queryStats.values()).reduce(
      (dist, stats) => {
  ...
  private analyzeCacheableQueries(): OptimizationSuggestion[] {
    ...
    const frequentSelects = Array.from(this.queryStats.values())
      .filter(
        stats =>
          stats.queryType === 'SELECT' && stats.executionCount >= 10 && stats.avgExecutionTime > 100
      )
      .sort((a, b) => b.executionCount - a.executionCount)
      .slice(0, 5);
  ...
  private analyzeQueryRewriteOpportunities(): OptimizationSuggestion[] {
    ...
    // 查找使用SELECT *的查询
    const selectAllQueries = Array.from(this.queryStats.values()).filter(stats =>
      stats.sql.includes('select *')
    );
  ...
  public getCacheStats() {
    ...
    const expiredEntries = Array.from(this.queryCache.values()).filter(
      entry => Date.now() - entry.timestamp.getTime() > entry.ttl * 1000
    ).length;
