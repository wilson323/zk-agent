...
class DatabasePerformanceMonitor extends EventEmitter {
  ...
  recordQuery(queryInfo) {
    ...
    if (duration > this.options.slowQueryThreshold) {
      ...
      logger.warn(`慢查询检测: ${duration}ms - ${queryInfo.query}`);
    }
  ...
  recordConnection(connectionInfo) {
    ...
    if (
      ...
      connectionInfo.poolStatus.totalCount > this.metrics.performance.peakConnections
    ) {
  ...
  getPerformanceStats() {
    ...
    const recentErrorRate =
      recentQueries.length > 0 ? recentErrors.length / recentQueries.length : 0;
