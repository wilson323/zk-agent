...
export class ConnectionPoolEnhancer extends EventEmitter {
    ...
    return [
      {
        ...
        condition: metrics => {
          return (
            metrics.cpuUsage > 80 || metrics.waitingRequests > 10 || metrics.avgResponseTime > 1000
          );
        },
        action: config => ({
          ...config,
  ...
  public async optimizePool(metrics: LoadMetrics): Promise<void> {
    ...
    try {
      ...
      // 查找匹配的策略
      const applicableStrategy = sortedStrategies.find(strategy => strategy.condition(metrics));
   ...
   * @returns 优化原因
   */
  private generateOptimizationReason(strategy: OptimizationStrategy, metrics: LoadMetrics): string {
    const reasons = [];
  ...
  public getOptimizationStats() {
    ...
    const strategyStats = this.optimizationHistory.reduce(
      (stats, record) => {
    ...
    const recentOptimizations = this.optimizationHistory.filter(
      record => Date.now() - record.timestamp.getTime() < 24 * 60 * 60 * 1000
    ).length;
        ...
        responseTime:
          (
            ((baselineResults.avgResponseTime - optimizedResults.avgResponseTime) /
              baselineResults.avgResponseTime) *
            100
          ).toFixed(2) + '%',
        throughput:
          (
        ...
        connectionUtilization:
          (
            ((optimizedResults.connectionUtilization - baselineResults.connectionUtilization) /
              baselineResults.connectionUtilization) *
            100
          ).toFixed(2) + '%',
      };
