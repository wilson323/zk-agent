...
export abstract class BaseModel extends EventEmitter {
  ...
  static belongsToMany(
    ...
    this.relations.set(relationName, {
      ...
      model,
      pivotTable:
        pivotTable || [this.name.toLowerCase(), model.name.toLowerCase()].sort().join('_'),
  ...
  protected async performUpdate(): Promise<void> {
    ...
    const query = QueryBuilderFactory.table(connection, config.tableName).where(
      primaryKey,
      '=',
      this.getKey()
    );
...
export function BelongsToMany(
  ...
  return function (target: any, propertyKey: string) {
    ...
    (target.constructor as typeof BaseModel).relations.set(relationName, {
      ...
      pivotTable:
        pivotTable ||
        [target.constructor.name.toLowerCase(), model.name.toLowerCase()].sort().join('_'),
