  ...
  production: {
    ...
    dialectOptions: {
      ...
      idle_in_transaction_session_timeout: parseInt(
        process.env.DB_IDLE_TRANSACTION_TIMEOUT || '60000'
      ),
      // 连接安全选项
      ...
      datasources: {
        db: {
          url:
            ...
            'postgresql://postgres:123456@localhost:5432/zk-agent-test',
        },
      },
    },
    // 测试环境安全配置
...
function validateDatabaseConfig(env = process.env.NODE_ENV || 'development') {
  ...
  if (env === 'production') {
    ...
    if (
      envConfig.password &&
      !/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*])/.test(envConfig.password)
    ) {
...
function getPrismaConfig(env = process.env.NODE_ENV || 'development') {
  ...
  return {
    datasourceUrl:
      process.env.DATABASE_URL || 'postgresql://postgres:123456@localhost:5432/zk-agent',
    log:
      envConfig.prisma?.logLevel === 'info'
