...
export class DeviceRecognitionEngine {
  ...
  private findNearbyEntities(
    ...
    return entities.filter(entity => {
      ...
      const distance = Math.sqrt(
        Math.pow(pos.x - centerPos.x, 2) + Math.pow(pos.y - centerPos.y, 2)
      );
  ...
  private calculateAIConfidence(device: DeviceInfo): number {
    ...
    // 基于几何匹配度
    if (device.geometricMatch) {
      confidence += 0.2;
    }

    // 基于文本匹配度
    if (device.textualMatch) {
      confidence += 0.2;
    }

    // 基于上下文匹配度
    if (device.contextualMatch) {
      confidence += 0.1;
    }
  ...
  private findPatternByBlockName(blockName: string): DevicePattern | null {
    return this.devicePatterns.find(pattern =>
      pattern.patterns.textual.some(textPattern =>
        textPattern.keywords.some(keyword =>
          blockName.toLowerCase().includes(keyword.toLowerCase())
        )
      )
    );
  }
  ...
  private findRelatedDevices(device: DeviceInfo, allDevices: DeviceInfo[]): string[] {
    return allDevices
      .filter(d => d.id !== device.id && this.areDevicesRelated(device, d))
      .map(d => d.id);
  }
  ...
  private areDevicesRelated(device1: DeviceInfo, device2: DeviceInfo): boolean {
    ...
    const distance = Math.sqrt(
      Math.pow(device1.location.x - device2.location.x, 2) +
        Math.pow(device1.location.y - device2.location.y, 2)
    );
