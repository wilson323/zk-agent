...
export class CADFileProcessor {
  ...
  private estimateFileComplexity(metadata: any): 'low' | 'medium' | 'high' {
    ...
    if (size > LARGE_FILE_SIZE || entityCount > LARGE_ENTITY_COUNT) {
      return 'high';
    }
    if (size > MEDIUM_FILE_SIZE || entityCount > MEDIUM_ENTITY_COUNT) {
      return 'medium';
    }
    return 'low';
  }
  ...
  ): Promise<Map<string, CADFileMetadata | { error: string }>> {
