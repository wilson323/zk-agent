...
export const useTouchOptimization = (
  ...
  const triggerHapticFeedback = useCallback(
    (type: 'light' | 'medium' | 'heavy' = 'light') => {
      if (!enableHapticFeedback || !isTouchDevice) {
        return;
      }
  ...
  const isAccidentalTap = useCallback(
    (startPoint: TouchPoint, endPoint: TouchPoint): boolean => {
      if (!preventAccidentalTaps) {
        return false;
      }
  ...
  const handleTouchMove = useCallback(
    (e: TouchEvent) => {
      if (!isTouchDevice || !touchStartRef.current) {
        return;
      }

      const touch = e.touches[0];
      if (!touch || touch.identifier !== touchStartRef.current.identifier) {
        return;
      }
...
export const useTouchGestures = (options: {
  ...
  useEffect(() => {
    if (!isTouchDevice || !elementRef.current) {
      return;
    }
