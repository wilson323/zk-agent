...
export class ContextMemoryManager {
  ...
  async compressSession(session: ContextSession): Promise<CompressionResult> {
    ...
    const importantMessages = messages.filter(
      msg => msg.role === 'system' || msg.metadata?.isImportant || msg.metadata?.isWelcome
    );
    ...
    // 创建摘要
    const removedMessages = messages.filter(msg => !preservedMessages.some(pm => pm.id === msg.id));
    ...
    const compressedTokens = session.messages.reduce(
      (total, msg) => total + (msg.metadata?.tokens || this.estimateTokens(msg.content)),
      0
    );
  ...
  async extractMemoryFragments(
    ...
    // 提取偏好信息
    const preferencePatterns = [/我喜欢|我偏好|我倾向于|我习惯/g, /我不喜欢|我讨厌|我不习惯/g];

    // 提取事实信息
    const factPatterns = [/我是|我叫|我的名字是|我来自/g, /我在|我工作在|我学习在/g];

    // 提取技能信息
    const skillPatterns = [/我会|我能够|我擅长|我精通/g, /我不会|我不擅长|我不熟悉/g];
    ...
    for (const pattern of factPatterns) {
      ...
      if (matches) {
        fragments.push(
          this.createMemoryFragment(session.id, session.userId, 'fact', message.content, 0.9, 0.9)
        );
      }
    }
    ...
    for (const pattern of skillPatterns) {
      ...
      if (matches) {
        fragments.push(
          this.createMemoryFragment(session.id, session.userId, 'skill', message.content, 0.8, 0.8)
        );
      }
    }
  ...
  async cleanupExpiredSessions(): Promise<number> {
    ...
    for (const session of allSessions) {
      const daysSinceLastActive =
        (now.getTime() - session.metadata.lastActiveAt.getTime()) / (1000 * 60 * 60 * 24);
  ...
  private shouldCompress(session: ContextSession): boolean {
    ...
    return (
      tokenRatio > session.config.compressionThreshold ||
  ...
  private extractTags(content: string): string[] {
    ...
    // 简单的标签提取
    if (contentLower.includes('工作') || contentLower.includes('职业')) {
      tags.push('工作');
    }
    if (contentLower.includes('学习') || contentLower.includes('教育')) {
      tags.push('学习');
    }
    if (contentLower.includes('爱好') || contentLower.includes('兴趣')) {
      tags.push('爱好');
    }
    if (contentLower.includes('家庭') || contentLower.includes('家人')) {
      tags.push('家庭');
    }
    if (contentLower.includes('技能') || contentLower.includes('能力')) {
      tags.push('技能');
    }
...
// 导出类型
export type { ChatMessage, ContextSession, MemoryFragment, CompressionResult };
