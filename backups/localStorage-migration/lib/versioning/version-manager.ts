...
export class VersionManager {
  ...
  async createVersion(
    ...
    try {
      ...
      const diff = latestVersion
        ? await this.calculateDiff(latestVersion.content, content)
        : undefined;
  ...
  async getVersion(versionId: string): Promise<ContentVersion | null> {
    try {
      ...
      // 从存储加载
      version = await this.loadVersion(versionId) || undefined;
      if (version) {
  ...
  async getVersionStats(userId?: string): Promise<VersionStats> {
    try {
      ...
      const compressionRatio =
        snapshotSize > 0 ? (snapshotSize + incrementalSize) / snapshotSize : 1;
...
if (typeof window !== 'undefined') {
  setInterval(
    () => {
      // 这里可以添加全局清理逻辑
      console.log('Performing periodic version cleanup');
    },
    24 * 60 * 60 * 1000 // 24小时
  );
}
