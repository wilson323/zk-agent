    ...
    const init = async () => {
      try {
        ...
        if ((apiConfig && apiConfig.baseUrl && apiConfig.apiKey) || isConfigured) {
          ...
          if (DEFAULT_AGENT.config?.apiKey && DEFAULT_AGENT.config?.baseUrl) {
            try {
              ...
              const success = await configureApi(
                DEFAULT_AGENT.config.baseUrl,
                DEFAULT_AGENT.config.apiKey,
                true
              );
        ...
        toast({
          ...
          description:
            'An error occurred during application initialization, switched to local storage mode',
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
