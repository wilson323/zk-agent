 ...
 * @returns JSX元素或null
 */
export function DIInitializer({ showStatus = false, onInitialized, onError }: DIInitializerProps) {
  ...
  // 渲染状态显示界面
  return (
    <InitializationStatusDisplay status={mapStatus(status)} progress={progress} error={error} />
  );
}
...
export function useDIInitialization() {
  const [status, setStatus] = useState<DIInitializationStatus>(diInitializer.getStatus());
  const [isInitialized, setIsInitialized] = useState(diInitializer.isInitialized());
