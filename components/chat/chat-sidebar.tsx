...
// 使用memo优化收藏消息项组件
const FavoriteMessageItem = memo(
  ({ message, formatDate, copyToClipboard, handleUnfavoriteMessage }) => {
    return (
      <div className='p-3 rounded-lg mb-3 bg-white dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700'>
        ...
        <div className='mt-2 p-3 rounded bg-gray-50 dark:bg-gray-700/50 text-sm'>
          {message.content}
        </div>

        <div className='mt-2 flex justify-between items-center'>
          <div className='text-xs text-gray-500 dark:text-gray-400'>
            来自: {message.appName || '未知智能体'}
          </div>
...
export function ChatSidebar({ isOpen, onClose }: ChatSidebarProps) {
  ...
  useEffect(() => {
    const savePinnedSessionsScroll = () => {
      if (pinnedSessionsParentRef.current) {
        debouncedSaveScrollPosition(
          SCROLL_POSITION_KEYS.PINNED_SESSIONS,
          pinnedSessionsParentRef.current.scrollTop
        );
      }
    };
    ...
    const saveUnpinnedSessionsScroll = () => {
      if (unpinnedSessionsParentRef.current) {
        debouncedSaveScrollPosition(
          SCROLL_POSITION_KEYS.UNPINNED_SESSIONS,
          unpinnedSessionsParentRef.current.scrollTop
        );
      }
    };
    ...
    const saveFavoritesScroll = () => {
      if (favoritesParentRef.current) {
        debouncedSaveScrollPosition(
          SCROLL_POSITION_KEYS.FAVORITES,
          favoritesParentRef.current.scrollTop
        );
      }
    };
  ...
  useEffect(() => {
    ...
    const restoreScrollTimeout = setTimeout(() => {
      // 恢复置顶会话滚动位置
      if (
        pinnedSessionsParentRef.current &&
        pinnedSessions.length > 0 &&
        !initialScrollRestored.pinnedSessions
      ) {
      ...
      // 恢复非置顶会话滚动位置
      if (
        unpinnedSessionsParentRef.current &&
        unpinnedSessions.length > 0 &&
        !initialScrollRestored.unpinnedSessions
      ) {
  ...
  // 滚动到选中的会话
  useEffect(() => {
    if (!isOpen || !selectedSession || searchQuery) {
      return;
    }

    // 查找选中会话在哪个列表中
    const isPinned = selectedSession.pinned;
    const containerRef = isPinned
      ? pinnedSessionsParentRef.current
      : unpinnedSessionsParentRef.current;
  ...
  const loadMoreSessions = useCallback(async () => {
    if (!selectedApp || isLoadingMore || !hasMoreSessions || searchQuery) {
      return;
    }
  ...
  const loadMoreFavorites = useCallback(() => {
    if (isLoadingMoreFavorites || !hasMoreFavorites) {
      return;
    }
        ...
        {activeTab === 'chats' ? (
          <>
            ...
            <div className='overflow-y-auto flex-1' ref={unpinnedSessionsParentRef}>
              ...
              {hasMoreSessions && (
                <LoadMoreIndicator loading={isLoadingMore} onInView={loadMoreSessions} />
              )}
            </div>
          </>
        ) : (
