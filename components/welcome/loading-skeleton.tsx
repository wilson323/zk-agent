...
const ListSkeleton = memo<{ count: number }>(({ count }) => (
  <div className='space-y-4'>
    {Array.from({ length: count }).map((_, i) => (
      <div
        key={i}
        className='flex items-center space-x-4 p-4 rounded-lg border bg-white dark:bg-gray-800'
      >
...
// 主骨架屏组件
const LoadingSkeleton = memo<LoadingSkeletonProps>(({ type, count = 6, className = '' }) => {
