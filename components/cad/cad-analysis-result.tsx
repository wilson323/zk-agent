...
import {
  exportCADAnalysis,
  shareCADAnalysis,
  advancedExportCADAnalysis,
} from '@/lib/cad/analysis-api';
          ...
          <div className='flex justify-between items-start'>
            <div>
              ...
              <CardDescription className='mt-1'>
                文件: {analysisData.fileName} ({analysisData.fileSize}) · 分析完成时间:{' '}
            ...
            <div className='flex gap-2'>
              <Button
                variant='outline'
                size='sm'
                onClick={() => copyToClipboard(JSON.stringify(analysisData))}
              >
                      ...
                      <ul className='space-y-3'>
                        {analysisData.summary.issues.map((issue, index) => (
                          <li key={index} className='flex items-center gap-2'>
                            <AlertCircle
                              className={cn('h-5 w-5', getSeverityColor(issue.severity))}
                            />
                      ...
                      <div key={index} className='border rounded-lg p-4'>
                        <div className='flex justify-between items-center'>
                          <h4 className='font-medium flex items-center gap-2'>
                            ...
                            {assembly.name}{' '}
                            <span className='text-muted-foreground text-sm'>({assembly.id})</span>
                          </h4>
                          <div className='flex items-center gap-2'>
                            <Badge variant='outline'>{assembly.parts} 零件</Badge>
                            <Badge variant='outline'>{assembly.subAssemblies} 子装配</Badge>
                            {assembly.issues > 0 && (
                              <Badge variant='destructive'>{assembly.issues} 问题</Badge>
                            )}
                          </div>
                        </div>
                      </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
                    ...
                    <div className='text-center p-3 border rounded-lg'>
                      <div className='text-2xl font-bold text-primary'>
                        {result.statistics?.complexity || 'N/A'}
                      </div>
