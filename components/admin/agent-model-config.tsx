    ...
    <div className='space-y-6'>
      ...
      {agentType === 'cad' && (
        <Card>
          ...
          <CardContent className='space-y-6'>
            {Object.entries(MODEL_PURPOSES.cad).map(([purpose, config]) =>
              renderModelSelector(purpose, config)
            )}
          </CardContent>
        </Card>
      )}
      ...
      {agentType === 'poster' && (
        <Card>
          ...
          <CardContent className='space-y-6'>
            {Object.entries(MODEL_PURPOSES.poster).map(([purpose, config]) =>
              renderModelSelector(purpose, config)
            )}
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className='space-y-6'>
          {Object.entries(MODEL_PURPOSES.common).map(([purpose, config]) =>
            renderModelSelector(purpose, config)
          )}
        </CardContent>
      </Card>
            ...
            {agentType === 'cad' && value.cadModels && (
              <>
                ...
                {Object.entries(value.cadModels).map(([purpose, modelId]) => {
                  ...
                  const purposeConfig =
                    MODEL_PURPOSES.cad[purpose as keyof typeof MODEL_PURPOSES.cad];
                  return (
            ...
            {agentType === 'poster' && value.posterModels && (
              <>
                ...
                {Object.entries(value.posterModels).map(([purpose, modelId]) => {
                  ...
                  const model = availableModels.find(m => m.id === modelId);
                  const purposeConfig =
                    MODEL_PURPOSES.poster[purpose as keyof typeof MODEL_PURPOSES.poster];
                  return (
                    <div key={purpose} className='flex justify-between pl-4'>
                      <span>{purposeConfig?.label}:</span>
                      <span className='font-medium'>{model?.name || '未知模型'}</span>
            ...
            {value.commonModels && (
              <>
                ...
                {Object.entries(value.commonModels).map(([purpose, modelId]) => {
                  ...
                  const model = availableModels.find(m => m.id === modelId);
                  const purposeConfig =
                    MODEL_PURPOSES.common[purpose as keyof typeof MODEL_PURPOSES.common];
                  return (
                    <div key={purpose} className='flex justify-between pl-4'>
                      <span>{purposeConfig?.label}:</span>
                      <span className='font-medium'>{model?.name || '未知模型'}</span>
