      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          toast({
            ...
            description:
              result.error || 'Unable to get available AI model list, using default models',
            variant: 'destructive',
          });
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          ...
          description:
            status === 'active' ? '智能体现在可供用户使用' : '智能体已被禁用，用户将无法访问',
        });
      } else {
        toast({
          title: '更新状态失败',
                    ...
                    <CardContent>
                      ...
                      <div className='flex items-center justify-between mb-4'>
                        ...
                        <span className='text-sm text-gray-500 dark:text-gray-400'>
                          {agent.type === 'fastgpt'
                            ? '使用FastGPT默认模型'
                            : models.find(m => m.id === agent.modelId)?.name ||
                              agent.modelId ||
                              '未指定'}
                        </span>
                      </div>
