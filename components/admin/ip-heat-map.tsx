  ...
  const fetchIPStats = async () => {
    try {
      ...
      const response = await fetch(
        `/api/admin/ip-stats?range=${timeRange}&province=${_selectedProvince}`
      );
      if (response.ok) {
        ...
        <CardHeader>
          <div className='flex justify-between items-start'>
            <div>
              ...
              <CardDescription>实时用户地理分布统计和热点分析</CardDescription>
            </div>
          ...
          {isLoading ? (
            ...
            <div className='space-y-6'>
                    ...
                    >
                      ...
                      <div className='flex items-center gap-2'>
                        <div className='text-right'>
                          <div className='font-medium'>{location.percentage.toFixed(1)}%</div>
                        ...
                        <div
                          className={`w-3 h-3 rounded-full ${getHeatColor(location.percentage)}`}
                        />
                      </div>
                    </motion.div>
                  ))}
              ...
              <div className='text-center text-sm text-gray-500'>
                最后更新:{' '}
                {ipStats?.lastUpdate ? new Date(ipStats.lastUpdate).toLocaleString() : '未知'}
              </div>
            </div>
          )}
        </CardContent>
