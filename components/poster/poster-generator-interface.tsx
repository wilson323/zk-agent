            ...
            <TemplateSelector
              onTemplateSelect={template => {
                ...
                if (template.metadata.tags.length > 0) {
                  setDescription(
                    prev => prev || `创建一个${template.name}风格的海报，${template.description}`
                  );
                }
                ...
                >
                  ...
                  <div className='flex items-center justify-center gap-2 text-sm text-gray-500'>
                    ...
                    <span>
                      运用
                      {selectedStyle
                        ? POSTER_STYLES.find(s => s.id === selectedStyle)?.name
                        : '智能'}
                      风格
                    </span>
                  </div>
                  ...
                  <div className='mt-4 flex justify-center gap-2 flex-wrap'>
                    {selectedStyle && (
                      <Badge variant='secondary'>
                        风格: {POSTER_STYLES.find(s => s.id === selectedStyle)?.name}
                      </Badge>
                    )}
                    {selectedSize && (
                      <Badge variant='secondary'>
                        尺寸: {POSTER_SIZES.find(s => s.id === selectedSize)?.name}
                      </Badge>
                    )}
                    {selectedPalette && (
                      <Badge variant='secondary'>
