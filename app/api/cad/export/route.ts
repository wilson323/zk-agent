...
import type { CADExportConfig } from "@/types/cad"
...
export const POST = createApiRoute(
  ...
  async (req: NextRequest, { params, validatedBody, validatedQuery, user, requestId }) => {
    try {
      ...
      const { analysisId, exportConfig }: { analysisId: string; exportConfig: CADExportConfig } = body
...
// 辅助函数：生成导出数据
async function generateExportData(analysisId: string, exportConfig: CADExportConfig) {
  // 模拟数据生成逻辑
  return {
